...
public:
    ...
    bool WriteZGRINCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZGRINCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZGRINCSeed_deprecated(uint256& seed);
    bool EraseZGRINCSeed();
    bool EraseZGRINCSeed_deprecated();

    bool WriteZGRINCCount(const uint32_t& nCount);
    bool ReadZGRINCCount(uint32_t& nCount);
