...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT5000IFMN = 3,
    ONLY_NONDENOMINATED_NOT5000IFMN = 4, // ONLY_NONDENOMINATED and not 5000 GRINC at the same time
    ONLY_5000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zGRINC send
enum ZerocoinSpendStatus {
    ZGRINC_SPEND_OKAY = 0,                            // No error
    ZGRINC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZGRINC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZGRINC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZGRINC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZGRINC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZGRINC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZGRINC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZGRINC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZGRINC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZGRINC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZGRINC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZGRINC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZGRINC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZGRINC_SPENT_USED_ZGRINC = 14,                      // Coin has already been spend
    ZGRINC_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZGRINC_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZGRINCBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZGRINCOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzGRINCAuto) const;
    ...
    */
    ...
    CzGRINCWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzGRINCTracker> zGRINCTracker;
    ...
    void setZWallet(CzGRINCWallet* zwallet)
    {
        zwalletMain = zwallet;
        zGRINCTracker = std::unique_ptr<CzGRINCTracker>(new CzGRINCTracker(strWalletFile));
    }

    CzGRINCWallet* getZWallet() { return zwalletMain; }
    ...
    void setZGRINCAutoBackups(bool fEnabled)
    {
    ...
    /** zGRINC reset */
    boost::signals2::signal<void()> NotifyzGRINCReset;
