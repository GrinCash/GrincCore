...
#ifndef GRINC_ZGRINCWALLET_H
#define GRINC_ZGRINCWALLET_H
...
public:
    CzGRINCWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZGRINC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZGRINC(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //GRINC_ZGRINCWALLET_H
