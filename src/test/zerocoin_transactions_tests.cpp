...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zGRINCTracker = unique_ptr<CzGRINCTracker>(new CzGRINCTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZGRINC_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZGRINC_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
