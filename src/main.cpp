...
#if defined(NDEBUG)
#error "GRINC cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uGRINC) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
   ...
   if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
       return error("%s : zGRINC spend with serial %s is already in block %d\n", __func__,
...
{
    //Check to see if the zGRINC is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zGRINC spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zGRINC without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zGRINC spend with serial %s from tx %s is not in valid range\n", __func__,
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
    ...
    if (GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE) && tx.ContainsZerocoins())
        return state.DoS(10, error("AcceptToMemoryPool : Zerocoin transactions are not supported by GRINC"), REJECT_INVALID, "bad-tx");
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zGRINC spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zGRINC");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zGRINC mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 1) {
        nSubsidy = 1300000 * COIN;  //premine
    } else if(nHeight >= 2 && nHeight <= 1999) { 
        nSubsidy = 1 * COIN;
    } else if(nHeight >= 2000 && nHeight <= 20999) { 
        nSubsidy = 22 * COIN;
    } else if(nHeight >= 21000 && nHeight <= 27999) {
        nSubsidy = 45 * COIN;
    } else if(nHeight >= 28000 && nHeight <= 34999) {
        nSubsidy = 55 * COIN;
    } else if(nHeight >= 35000 && nHeight <= 41999) {
        nSubsidy = 65 * COIN;
    } else if(nHeight >= 42000 && nHeight <= 48999) {
        nSubsidy = 55 * COIN;
    } else if(nHeight >= 49000 && nHeight <= 59999) {
        nSubsidy = 52 * COIN;
    } else if(nHeight >= 60000 && nHeight <= 89999) {
        nSubsidy = 40 * COIN;
    } else if(nHeight >= 90000 && nHeight <= 119999) {
        nSubsidy = 45 * COIN;
    } else if(nHeight >= 120000 && nHeight <= 179999) {
        nSubsidy = 35 * COIN;
    } else if(nHeight >= 180000 && nHeight <= 349999) {
        nSubsidy = 20 * COIN;
    } else if(nHeight >= 350000 && nHeight <= 699999) {
        nSubsidy = 15 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZGRINCStake)
{
    int64_t ret = 0;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from GRINC
...
{
    RenameThread("GRINC-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZGRINCMinted()
{
...
void RecalculateZGRINCSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zGRINC supply
...
bool RecalculateGRINCSupply(int nHeightStart)
{
...
{
    // GRINC: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZGRINCSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (block.nTime > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE) && !IsInitialBlockDownload() && tx.ContainsZerocoins()) {
            return state.DoS(100, error("ConnectBlock() : zerocoin transactions are not supported by GRINC"));
        }
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        RecalculateZGRINCMinted();
        RecalculateZGRINCSpent();
        RecalculateGRINCSupply(Params().Zerocoin_StartHeight());
    }

    //Track zGRINC money supply in the block index
    if (!UpdateZGRINCSupply(block, pindex, fJustCheck))
        return state.DoS(100, error("%s: Failed to calculate new zGRINC supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zGRINCSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert GRINC to zGRINC
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zGRINC spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn& txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zGRINC serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zGRINC stake block is less than allowed start height", __func__);

    if (CZGRINCStake* zGRINC = dynamic_cast<CZGRINCStake*>(stake)) {
        CBlockIndex* pindexFrom = zGRINC->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zGRINC stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
             return error("%s: zGRINC stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zGRINC->GetValue()));
        if (nChecksum200 != zGRINC->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zGRINC->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZGRINC() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zGRINC fails context checks", __func__));
    ...
    if (isPoS) {
       ...
       // Inputs
       std::vector<CTxIn> GRINCInputs;
       std::vector<CTxIn> zGRINCInputs;

       for (const CTxIn& stakeIn : stakeTxIn.vin) {
           if(stakeIn.scriptSig.IsZerocoinSpend()){
               zGRINCInputs.push_back(stakeIn);
           }else{
               GRINCInputs.push_back(stakeIn);
           }
       }
       const bool hasGRINCInputs = !GRINCInputs.empty();
       const bool hasZGRINCInputs = !zGRINCInputs.empty();
       ...
       for (const CTransaction& tx : block.vtx) {
           for (const CTxIn& in: tx.vin) {
               ...
               if(hasGRINCInputs)
                   ...
                   for (const CTxIn& GRINCIn : GRINCInputs){
                       if(GRINCIn.prevout == in.prevout){
       ...
       if (isBlockFromFork) {
           ...
           do {
               ...
               for (const CTransaction& t : bl.vtx) {
                   for (const CTxIn& in: t.vin) {
                       ...
                       for (const CTxIn& stakeIn : GRINCInputs) {
           ...
           // Now that this loop if completed. Check if we have zGRINC inputs.
           if(hasZGRINCInputs){
               for (const CTxIn& zPivInput : zGRINCInputs) {
                   ...
                   if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, spend, pindex, 0))
                       return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                  stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zGRINC");
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zGRINC mints and %d zGRINC spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // GRINC: We use certain sporks during IBD, so check to see if they are
