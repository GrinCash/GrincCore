...
#include "ui_zGRINCcontroldialog.h"
...
std::set<std::string> ZGRINCControlDialog::setSelectedMints;
std::set<CMintMeta> ZGRINCControlDialog::setMints;

bool CZGRINCControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZGRINCControlDialog::COLUMN_DENOMINATION || column == ZGRINCControlDialog::COLUMN_VERSION || column == ZGRINCControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZGRINCControlDialog::ZGRINCControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZGRINCControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZGRINCControlDialog::~ZGRINCControlDialog()
{
    delete ui;
}

void ZGRINCControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZGRINCControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZGRINCControlWidgetItem* itemDenom(new CZGRINCControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZGRINCControlWidgetItem *itemMint = new CZGRINCControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
...
void ZGRINCControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZGRINCControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZGRINC_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZGRINCControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZGRINCControlDialog::GetSelectedMints()
{
...
void ZGRINCControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
