...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::GRINC);
    ...
    if (!settings.contains("nAnonymizeGRINCAmount"))
        settings.setValue("nAnonymizeGRINCAmount", 1000);

    nAnonymizeGRINCAmount = settings.value("nAnonymizeGRINCAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeGRINCAmount"))
        SoftSetArg("-anonymizeGRINCamount", settings.value("nAnonymizeGRINCAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in GRINC.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeGRINCAmount:
            return QVariant(nAnonymizeGRINCAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeGRINCAmount:
            nAnonymizeGRINCAmount = value.toInt();
            settings.setValue("nAnonymizeGRINCAmount", nAnonymizeGRINCAmount);
            emit anonymizeGRINCAmountChanged(nAnonymizeGRINCAmount);
            break;
        case CoinControlFeatures:
