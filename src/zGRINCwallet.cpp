...
CzGRINCWallet::CzGRINCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zGRINC seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZGRINCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZGRINCSeed_deprecated()) {
                    LogPrintf("%s: Updated zGRINC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zGRINC seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zGRINC wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzGRINCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZGRINCCount(nCountLastUsed);
    else if (!walletdb.ReadZGRINCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzGRINCWallet::Lock()
{
    seedMaster = 0;
}

void CzGRINCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzGRINCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZGRINC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzGRINCWallet::LoadMintPoolFromDB()
{
...
void CzGRINCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzGRINCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzGRINCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zGRINCTracker->HasPubcoinHash(pMint.first)) {
...
bool CzGRINCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZGRINC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zGRINCTracker which also adds to database
    pwalletMain->zGRINCTracker->Add(dMint, true);
...
void CzGRINCWallet::SeedToZGRINC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzGRINCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzGRINCWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZGRINCCount(nCountLastUsed);
...
void CzGRINCWallet::GenerateDeterministicZGRINC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzGRINCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzGRINCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
