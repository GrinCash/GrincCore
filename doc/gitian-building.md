...
*Setup instructions for a gitian build of GRINC using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the GRINC
...
Clone the git repositories for GRINC and gitian and then checkout the GRINC version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/GRINC-crypto/GRINC.git
cd GRINC
...
Follow the instructions in [doc/release-process.md](release-process.md) in the GRINC repository
...
To build GRINC (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the GRINC repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/GRINC/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/GRINC-crypto/GRINC
...
For example:
```bash
URL=https://github.com/crowning-/GRINC.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit GRINC=${COMMIT} --url GRINC=${URL} ../GRINC/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit GRINC=${COMMIT} --url GRINC=${URL} ../GRINC/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit GRINC=${COMMIT} --url GRINC=${URL} ../GRINC/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/GRINC-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/GRINC-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/GRINC-build.assert
```
...
[GRINC/gitian.sigs](https://github.com/GRINC-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
